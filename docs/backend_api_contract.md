...
## 2. ✅ Политика отмены — РЕАЛИЗОВАНО (включая per-doctor)

`AppointmentSerializer` отдаёт вычисленные `can_cancel` и `can_reschedule`;
мобильный клиент использует их (с фолбэком на локальное правило).

Окно отмены **настраивается per-doctor**: поле `cancellation_window_hours` в
`DoctorProfile` (значения 1/2/6/12/24, дефолт 2), редактируется врачом в профиле
(`PATCH /doctor/profile/`) и отдаётся в `/auth/me`. Cancel/reschedule-вью и
`can_cancel`/`can_reschedule` берут окно из профиля врача (фолбэк — константа
