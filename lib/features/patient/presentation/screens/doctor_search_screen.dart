                ...
                DoctorSearchStatus.loadingMore =>
                  results.doctors.isEmpty
                      ...
                      : Builder(builder: (context) {
                          ...
                          // ordering=next_slot already carries next_slot_at
                          // per doctor (see _DoctorCard) — only the doctors
                          // still missing it need the batched fetch.
                          final needsBatch = sorted
                              .where((d) => d.nextSlotAt == null)
                              .map((d) => d.id)
                              .toList();
                          final nextSlotBatch = needsBatch.isEmpty
                              ? const AsyncValue<Map<String, DateTime?>>.data(
                                  {})
                              : ref.watch(nextSlotBatchProvider(
                                  NextSlotBatchParams(needsBatch)));
                          final isLoadingMore =
...
class _DoctorCard extends ConsumerWidget {
  const _DoctorCard({required this.doctor, required this.batchedNextSlot});
  final DoctorModel doctor;

  /// The whole screen's batched next-slot lookup (see
  /// `nextSlotBatchProvider`) — this card just reads its own id out of it,
  /// rather than each card making its own request.
